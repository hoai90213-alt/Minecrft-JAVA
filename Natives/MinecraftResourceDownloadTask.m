...
- (instancetype)init {
    ...
    self.manager.completionQueue = dispatch_queue_create("org.angelauramc.amethyst.download", DISPATCH_QUEUE_CONCURRENT);
    self.manager.operationQueue.maxConcurrentOperationCount = 8;
...
- (void)downloadModpackFromAPI:(ModpackAPI *)api detail:(NSDictionary *)modDetail atIndex:(NSUInteger)selectedVersion {
    ...
    NSArray *versionHashes = modDetail[@"versionHashes"];
    if (selectedVersion >= versionURLs.count || selectedVersion >= versionSizes.count || selectedVersion >= versionHashes.count) {
        [self finishDownloadWithErrorString:@"Invalid modpack version selection."];
        return;
    }

    NSString *url = versionURLs[selectedVersion];
    NSUInteger size = [versionSizes[selectedVersion] unsignedLongLongValue];
    NSString *sha = versionHashes[selectedVersion];
    if (url.length == 0) {
        [self finishDownloadWithErrorString:@"Invalid modpack package URL."];
        return;
    }
...
- (BOOL)checkSHAIgnorePref:(NSString *)sha forFile:(NSString *)path altName:(NSString *)altName logSuccess:(BOOL)logSuccess {
    ...
    if (stream.streamStatus == NSStreamStatusError) {
        NSLog(@"[MCDL] SHA1 checker: failed to open %@: %@", altName ? altName : path.lastPathComponent, stream.streamError.localizedDescription);
        [stream close];
        return NO;
    }
    ...
    while (YES) {
        ...
        if (bytesRead < 0) {
            NSLog(@"[MCDL] SHA1 checker: failed to read %@: %@", altName ? altName : path.lastPathComponent, stream.streamError.localizedDescription);
            [stream close];
            return NO;
        }
        if (bytesRead == 0) {
        ...
        CC_SHA1_Update(&context, buffer, (CC_LONG)bytesRead);
    }
    if (stream.streamStatus == NSStreamStatusError) {
        NSLog(@"[MCDL] SHA1 checker: read error for %@: %@", altName ? altName : path.lastPathComponent, stream.streamError.localizedDescription);
        [stream close];
        return NO;
    }
    [stream close];
