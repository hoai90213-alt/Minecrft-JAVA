...
static const NSUInteger kMaxBufferedLogLines = 2000;
...
- (void)actionToggleLogOutput {
    ...
    [UIView transitionWithView:self duration:0.4 options:opt animations:^(void){
        ...
        if (!self.navController.view.hidden) {
            [self.logTableView reloadData];
            if (logLines.count > 0) {
                NSIndexPath *indexPath = [NSIndexPath indexPathForRow:logLines.count - 1 inSection:0];
                [self.logTableView scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionBottom animated:NO];
            }
        }
    }];
}
...
+ (void)_appendToLog:(NSString *)line {
    ...
    if (logLines.count > kMaxBufferedLogLines) {
        [logLines removeObjectsInRange:NSMakeRange(0, logLines.count - kMaxBufferedLogLines)];
        didTrim = YES;
    }
    if (!current || !current.logTableView || current.navController.view.hidden) {
        return;
    }

    if (didTrim) {
        [current.logTableView reloadData];
        NSIndexPath *lastIndexPath = [NSIndexPath indexPathForRow:logLines.count - 1 inSection:0];
        [current.logTableView scrollToRowAtIndexPath:lastIndexPath atScrollPosition:UITableViewScrollPositionBottom animated:NO];
        return;
    }
