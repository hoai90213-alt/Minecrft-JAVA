...
@property(nonatomic, weak) NSProgress* observedTaskProgress;
...
- (void)observeTaskProgress {
    NSProgress *progress = self.task.progress;
    if (!progress || progress == self.observedTaskProgress) {
        return;
    }
    [self removeTaskProgressObserver];
...
- (void)removeTaskProgressObserver {
    if (!self.observedTaskProgress) {
        return;
    }
    @try {
        [self.observedTaskProgress removeObserver:self forKeyPath:@"fractionCompleted"];
...
- (void)launchMinecraft:(UIButton *)sender {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        self.task.handleError = ^{
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf removeTaskProgressObserver];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if (!self.task || !self.task.progress || !self.task.textProgress) {
        return;
    }
    ...
    if (lastSecTime < tv.tv_sec) {
        ...
        CGFloat elapsed = currentTime - lastMsTime;
        NSInteger deltaCompleted = completedUnitCount - lastCompletedUnitCount;
        NSInteger throughput = (elapsed > 0 && deltaCompleted > 0) ? deltaCompleted / elapsed : 0;
        progress.throughput = @(MAX(throughput, 0));
