...
- (void)loadDetailsOfMod:(NSMutableDictionary *)item {
    ...
    [response enumerateObjectsUsingBlock:^(NSDictionary *version, __unused NSUInteger i, BOOL *stop) {
        NSString *versionName = version[@"name"];
        NSDictionary *file = [version[@"files"] firstObject];
        NSString *url = file[@"url"];
        if (versionName.length == 0 || url.length == 0) {
            return;
        }
    ...
    if (names.count == 0) {
        self.lastError = [NSError errorWithDomain:@"ModrinthAPIErrorDomain"
                                             code:1
                                         userInfo:@{NSLocalizedDescriptionKey: @"No downloadable versions were found for this project."}];
        return;
    }
