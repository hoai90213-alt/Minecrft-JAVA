...
void init_redirectStdio() {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        ssize_t rsize;
        char buf[2048];
        NSTimeInterval lastSyncTime = CFAbsoluteTimeGetCurrent();
        NSUInteger pendingSyncBytes = 0;
        while((rsize = read(pfd[0], buf, sizeof(buf)-1)) > 0) {
            @autoreleasepool {
                ...
                pendingSyncBytes += (NSUInteger)rsize;
                NSTimeInterval now = CFAbsoluteTimeGetCurrent();
                if (pendingSyncBytes >= 64 * 1024 || now - lastSyncTime >= 1.0) {
