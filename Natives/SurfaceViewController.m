...
@property(nonatomic) CADisplayLink *inputTickDisplayLink;
...
- (void)viewDidLoad
{
    ...
    self.inputTickDisplayLink = [CADisplayLink displayLinkWithTarget:tickInput selector:@selector(invoke)];
    if (@available(iOS 15.0, tvOS 15.0, *)) {
        if(getPrefBool(@"video.max_framerate")) {
            self.inputTickDisplayLink.preferredFrameRateRange = CAFrameRateRangeMake(30, 120, 120);
        } else {
            self.inputTickDisplayLink.preferredFrameRateRange = CAFrameRateRangeMake(30, 60, 60);
        }
    }
    [self.inputTickDisplayLink addToRunLoop:NSRunLoop.currentRunLoop forMode:NSRunLoopCommonModes];
...
- (void)dealloc {
    [self.inputTickDisplayLink invalidate];
    self.inputTickDisplayLink = nil;

    NSNotificationCenter *center = NSNotificationCenter.defaultCenter;
    if (self.mouseConnectCallback) {
        [center removeObserver:self.mouseConnectCallback];
    }
    if (self.mouseDisconnectCallback) {
        [center removeObserver:self.mouseDisconnectCallback];
    }
    if (self.controllerConnectCallback) {
        [center removeObserver:self.controllerConnectCallback];
    }
    if (self.controllerDisconnectCallback) {
        [center removeObserver:self.controllerDisconnectCallback];
    }
}
